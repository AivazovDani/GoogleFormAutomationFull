...
FORM_URL = 'https://docs.google.com/forms/d/e/1FAIpQLScgRAlvUvfy5-y9nkdc13VkpJ96Z26mTBXbVwebrekzdHzdwQ/viewform'
...
DATA = {
    ...
    "EIN/VAT (from Entity)": "Business EIN",
    "Use-Case": "Summarize your SMS Use-case",
    "Safe Page URL": "Opt-in URL",
    "Message Flow": "Opt-in Workflow",
    "Opt-In Screenshot":"Opt-in Workflow Image URL",
    "Registered copies": "Sample Message Content",
...
def extract_form_data(airtable_record, other_fields=None):
    ...
    if owner_name:
        parts = owner_name.strip().split(" ", 1)  # Split into max 2 parts
        form_data["First Name"] = parts[0]
        form_data["Last Name"] = parts[1] if len(parts) > 1 else ""

    # Owner Email
    real_part = clean_value(fields.get("Domain", "")).split("https://", 1)
    email = f"info@{real_part[1]}"
    ...
    # Business Name
    llc_name = clean_value(fields.get("Entity name", ""))
    form_data["Business Name"] = f"{llc_name} DBA {brand_name}".strip()


    # HardCoded Values
    form_data["company name"] = "ClickLab"
    form_data["Parent company"] = "N/A"
    form_data["messaging volume"] = "100k"
    form_data["Use-case"] = "Marketing & Account Notification"
    form_data["Additional Use-case Details"] = "Messages are sent to opted-in subscribers to confirm registrations, send reminders, and notify them of new opportunities. All messages include STOP to opt out"
